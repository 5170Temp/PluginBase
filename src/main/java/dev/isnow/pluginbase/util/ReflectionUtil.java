...
public class ReflectionUtil {
    ...
    public List<Class<?>> getClasses(final String packageName) throws ClassNotFoundException, IOException {
        ...
        final ClassLoader pluginClassLoader = PluginBase.class.getClassLoader();
        ...
        if (resource != null) {
            ...
            BaseLogger.error("Resource is null! " + packageName);
        }
    ...
    private void findClassesInJar(final JarFile jarFile, final String path, final List<Class<?>> classes) {
    ...
    public <T> Class<T> getGenericTypeClass(final Class<?> startClass, final Class<?> targetSuperClass, final int index) {
        Type currentType = startClass;
        Class<?> currentClass = startClass;

        while (currentClass != null && !currentClass.equals(targetSuperClass)) {
